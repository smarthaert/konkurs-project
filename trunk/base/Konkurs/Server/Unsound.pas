...
type
  ...
  TSndRec = record
    Vol, Frec : Array [0..SOUND_COUNT-1] of  smallint;
...
begin
   ...
   for i:=0 to SOUND_COUNT-1 do begin
     if NewRec.Vol[i]<>OldRec.Vol[i] then SB[i].Volume:=NewRec.Vol[i];
     if NewRec.Frec[i]<>OldRec.Frec[i] then SB[i].Frequency:=NewRec.Frec[i];
