unit UnTarget;   //Формирование плоских мишеней
...
begin
  result:='Грудная фигура';
  case Num of
    M6: result:='Грудная фигура';
    M7: result:='Поясная фигура';
    M8: result:='Ростовая фигура';
    M8A: result:='Ростовая фигура';
    M9: result:='Ручной противотанковый гранатомёт';
    M9A: result:='Противотанковый гранатомёт в окопе';
    M9C: result:='Расчёт ПТУР в окопе';
    M10: result:='Ручной пулемёт';
    M10A: result:='Пулемётный расчёт';
    M11: result:='Противотанковое (безоткатное) орудие';
    M12: result:='Танк';
    M12A: result:='Танк';
    M12B: result:='Танк в окопе';
    M13: result:='Бронетранспортёр';
    M13A: result:='Бронетранспортёр';
    M14: result:='Боевая машина пехоты';
    M17: result:='Безоткатное орудие на автомобиле';
    M17A: result:='Безоткатное орудие на автомобиле';
    M17B: result:='Безоткатное орудие на автомобиле в окопе';
    M18: result:='ПТУР на бронетранспортёре';
    M19: result:='Артиллерийское орудие';
    M20: result:='Самоходная ракетная установка';
    M20A: result:='Самоходная ракетная установка в пох.положении';
    M25: result:='Вертолёт';
  end;
end;
end.
