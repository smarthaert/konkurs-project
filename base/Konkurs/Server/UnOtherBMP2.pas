...
procedure SetDimensionDirectris(season,region : integer);
begin
//  posH:=posHvis-Surface.CountHeight(posX, posY);
...
begin
  for a:=1 to 3 do begin
    ...
    if a=2 then BMP[a].Htek:=Surface.CountHeight(BMP[a].Xtek,BMP[a].Ytek)
           else BMP[a].Htek:=-10;
  end;
end;
...
begin
  ...
  SetLength(Mestniks, Surface.colPatchSurface+1);
  if TypesOfOrient<>nil then TypesOfOrient.Free;
  TypesOfOrient:=TList.Create;
  for a:=1 to Surface.colPatchSurface do  begin
